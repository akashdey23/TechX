...
export default function Home() {
  ...
  const buttonAction = async (action, slug, initialQuantity) => {
    ...
    if(action=="Delete")
    {
      newDropdown[indexdrop].quantity=parseInt(initialQuantity=0);
          ...
          {dropdown.map(item => {
            return <div key={item.slug} className="container flex justify-between p-2 my-1 border-b-2">
              ...
              <div className='mx-5'>
                ...
                <button onClick={()=> { buttonAction("Delete", item.slug,item.quantity) }} disabled={loadingaction} className="add inline-block Delete px-3 py-1 cursor-pointer bg-purple-500 text-white font-semibold rounded-lg shadow-md disabled:bg-purple-200">Delete</button>
              </div>
            </div>
          })}
        </div>
